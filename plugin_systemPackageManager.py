...
class extension(dmenu_extended.dmenu):
    ...
    def install_package(self):
        ...
        if len(package) > 0:
            self.open_terminal(
                self.command_installPackage + package.split(" ")[0], True
            )
            self.rebuild_notice()
    ...
    def u_installedPackages_portage(self):
        os.system(
            "cd /var/db/pkg/ && ls -d */* > " + dmenu_extended.path_cache + "/tmp.txt"
        )
        packages = self.command_output("cat " + dmenu_extended.path_cache + "/tmp.txt")
        os.system("rm " + dmenu_extended.path_cache + "/tmp.txt")
        return packages
    ...
    def rebuild_notice(self):
        ...
        if self.prefs["terminal"] != "gnome-terminal":
            rebuild = self.menu(
                [
                    "Cache may be out-of-date, rebuild at your convenience.",
                    "* Rebuild cache now",
                ]
            )
    ...
    def run(self, inputText):
        if not self.detected_packageManager:
            ...
            print(
                "Detected system package manager as "
                + str(self.detected_packageManager)
            )

        items = [
            self.prefs["indicator_submenu"] + " Install a new package",
