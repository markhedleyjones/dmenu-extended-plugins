...
class extension(dmenu_extended.dmenu):
    ...
    def load_config(self):
        ...
        if os.path.isfile(path_config):
            ...
            for key in defaults["default_settings"]:
                if (
                    key not in tmp["default_settings"]
                    or tmp["default_settings"][key] != defaults["default_settings"][key]
                ):
                    match = False
    ...
    def settings(self):
        ...
        if self.journal_is_encrypted():
            options.append(
                self.prefs["indicator_submenu"]
                + " Decrypt journal (remove password for '"
            ...
            options.append(
                self.prefs["indicator_submenu"]
                + " Encrypt journal (add password to '"
                + self.current_journal
                + "')"
            )
    ...
    def add_entry(self):
        ...
        date = self.select(opts, prompt="Entry date: ")[
            len(self.prefs["indicator_submenu"]) + 1 : len(
                self.prefs["indicator_submenu"]
            )
            + 11
        ]
        ...
        while True:
            ...
            if len(journal_data) > 0 and self.bodies_current_journal_flag == True:
                if self.config["display_entry_titles_only"] == True:
                    ...
                    options.append(
                        self.prefs["indicator_submenu"] + " Show only titles"
                    )

            journal_switch_prefix = self.prefs["indicator_submenu"] + " Switch to '"

            for journal_name in [
                a for a in self.config["journals"] if a != self.current_journal
            ]:
            ...
            if out == options[0]:
                ...
                out == self.prefs["indicator_submenu"] + " Show full entries"
                or out == self.prefs["indicator_submenu"] + " Show only titles"
            ):
                ...
                try:
                    date_length = len(
                        datetime.datetime.today().strftime(self.config["timeformat"])
                    )
                    entry_date = datetime.datetime.strptime(
                        out[:date_length], self.config["timeformat"]
                    )
                except Exception:
                    pass
